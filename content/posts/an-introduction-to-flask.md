...
خب حالا باید ببینیم چرا به کلاس `Flask`, متغیر `__name__` رو میدیم. در واقع یکی از دلایل این کار اینه که فلسک بفهمه ریشه یا همون `instance` ما کجاست و بر اساس همون `template` ها و خیلی چیزای دیگه‌رو لود کنه. اگه یرنامه شما توی همین چند خط خلاصه می‌شه و به `Flask` هر اسم دیگه‌ای رو بدید احتمالا به مشکل خاصی بر نمی‌خورید.